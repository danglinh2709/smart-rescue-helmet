...
class SimulatorConfig:
    ...
    def from_env(
        ...
        source = values.get("SIMULATOR_SOURCE", "SIMULATOR")
        if source != "SIMULATOR":
            raise ValueError("SIMULATOR_SOURCE must be SIMULATOR")

        return cls(
            device_id=validate_device_id(
                values.get("SIMULATOR_DEVICE_ID", "FF01")
            ),
            mqtt_host=values.get("MQTT_HOST", "mosquitto"),
            mqtt_port=mqtt_port,
            telemetry_interval=_positive_float(
                values, "SIMULATOR_TELEMETRY_INTERVAL", "1"
            ),
            status_interval=_positive_float(
                values, "SIMULATOR_STATUS_INTERVAL", "2"
            ),
            health_interval=_positive_float(
                values, "SIMULATOR_HEALTH_INTERVAL", "5"
            ),
            source=source,
