...
class MqttPublisher:
    ...
    def _on_connect(
        ...
        if getattr(reason_code, "is_failure", reason_code != 0):
            ...
            LOGGER.info("mqtt connected host=%s port=%s", self._host, self._port)
    ...
    def connect(self) -> None:
        ...
        if self._connection_error is not None:
            self.disconnect()
            raise ConnectionError(f"MQTT connection rejected: {self._connection_error}")
    ...
    def _publish(
        ...
        validator = getattr(self._validator, f"validate_{message_type}")
        validator(payload)
        topic = build_topic(str(payload["device_id"]), message_type)
    ...
    def publish_health(self, payload: dict[str, Any]) -> None:
        self._publish("health", payload, qos=1)
        LOGGER.info(
