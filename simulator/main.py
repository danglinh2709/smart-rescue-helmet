...
class SimulatorRunner:
    def __init__(self, config: Any, factory: Any, publisher: Any) -> None:
    ...
    def publish_due(self, now: float) -> float:
        if self._next_publish is None:
            self._next_publish = {message_type: now for message_type in self._intervals}
    ...
    def run(self, stop_event: Event) -> None:
        ...
        while not stop_event.is_set():
            delay = max(0.0, next_deadline - monotonic())
...
def main() -> None:
    ...
    def request_shutdown(signum: int, frame: Any) -> None:
        LOGGER.info("simulator shutdown requested signal=%s", signum)
        stop_event.set()
    ...
    validator = ContractValidator(config.schema_dir)
    publisher = MqttPublisher(
        host=config.mqtt_host,
        port=config.mqtt_port,
        validator=validator,
    )
    runner = SimulatorRunner(config, factory, publisher)

    LOGGER.info(
        "simulator started device=%s scenario=NORMAL telemetry_interval=%s "
        "status_interval=%s health_interval=%s",
        config.device_id,
    ...
    try:
        ...
        LOGGER.exception("simulator stopped due to an unrecoverable error")
        raise
    finally:
        if connected:
            publisher.disconnect()
        LOGGER.info("simulator stopped device=%s", config.device_id)
